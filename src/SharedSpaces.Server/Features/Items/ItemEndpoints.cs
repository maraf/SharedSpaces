...
public static class ItemEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Read destination token claims without signature verification to extract server_url.
        // This is safe because the destination server will fully validate the token.
        var tokenHandler = new JwtSecurityTokenHandler { MapInboundClaims = false };
        JwtSecurityToken? jwtToken;
        try
        {
            jwtToken = tokenHandler.ReadJwtToken(request.DestinationToken);
        }
        ...
        var destinationServerUrl = jwtToken.Claims
            .FirstOrDefault(c => c.Type == SpaceMemberClaimTypes.ServerUrl)?.Value;

        // Compare server_url claims: source user's token was generated by this server,
        // so its server_url matches tokens from the same server.
        var sourceServerUrl = httpContext.User.FindFirst(SpaceMemberClaimTypes.ServerUrl)?.Value
            ?? $"{httpContext.Request.Scheme}://{httpContext.Request.Host}";
        var isCrossServer = !IsSameServer(sourceServerUrl, destinationServerUrl);
    ...
    private static bool IsSameServer(string sourceUrl, string? destinationUrl)
    {
        ...
        {
            // No server_url claim — assume same server (backwards compatibility)
            return true;
        }

        return string.Equals(
            sourceUrl.TrimEnd('/'),
            destinationUrl.TrimEnd('/'),
            StringComparison.OrdinalIgnoreCase);
    }

    private static async Task<IResult> TransferItemCrossServer(
        Guid spaceId,
        Guid itemId,
        string action,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(destinationServerUrl))
        {
            return Results.BadRequest(new { Error = "Destination token missing server_url claim" });
        }

        var destSpaceClaim = jwtToken.Claims
            .FirstOrDefault(c => c.Type == SpaceMemberClaimTypes.SpaceId)?.Value;
        ...
        // Reject same-space transfer (even cross-server, unlikely but guard against)
        ...
        // Build multipart form-data for the destination server's PUT endpoint
        ...
        if (isFile)
        {
            var sourceStream = await fileStorage.ReadAsync(spaceId, itemId, cancellationToken);
            ...
            var fileName = isAutoConvertedText ? $"{newItemId:N}.txt" : sourceItem.Content;
            formContent.Add(streamContent, "file", fileName);
        }
        else
        {
            formContent.Add(new StringContent(sourceItem.Content), "content");
        }

        // Upload to destination server
        using var httpClient = httpClientFactory.CreateClient();
        var destUrl = $"{destinationServerUrl.TrimEnd('/')}/v1/spaces/{destinationSpaceId}/items/{newItemId}";

        using var uploadRequest = new HttpRequestMessage(HttpMethod.Put, destUrl);
        ...
        {
            return Results.Json(
                new { Error = "Failed to connect to destination server" },
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            destinationPrincipal = tokenHandler.ValidateToken(destinationToken, new TokenValidationParameters
            {

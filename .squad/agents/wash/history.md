...
- **CI/CD workflows for client (2026-03-21):** Created `client-publish.yml` and `client-deploy.yml` workflows. Updated `vite.config.ts` to prefer `process.env.VITE_APP_VERSION` over `pkg.version` for version injection — package.json stays at `0.0.0`. Publish workflow: triggers on `client-*` tags, builds with VITE_APP_VERSION from tag, zips dist/, creates GH Release with `gh release create`. Deploy workflow: manual `workflow_dispatch` with `tag` input, checks gh-pages branch for CNAME to decide base path (custom domain → `/`, otherwise `/{repo-name}/`), uses `vite build --base` for base path, deploys via `actions/upload-pages-artifact` + `actions/deploy-pages`. Key files: `.github/workflows/client-publish.yml`, `.github/workflows/client-deploy.yml`, `src/SharedSpaces.Client/vite.config.ts`.

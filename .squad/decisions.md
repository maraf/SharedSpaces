...
### Issue #42 Research: Web Share Target API — Architecture

**Decision Date:** 2026-03-19  
**Decided By:** Mal (Lead)  
**Status:** Research Complete — Awaiting Product Clarification

#### Context
Issue #42 requires implementing Web Share Target API support, allowing users to share files/text directly from their OS's system share picker into SharedSpaces. This is a PWA feature with multiple architectural concerns spanning frontend (manifest, service worker, UI), backend (share endpoint), and infrastructure (HTTPS, installability).

#### Decisions

**Decision 1: Timing & Phase**
Keep Share Target in Phase 5 (after service worker work #28). Service worker is a shared dependency for both offline support and share target. Doing them together avoids split implementation work.

**Decision 2: Data Flow**
- Web Share Target POST arrives at service worker (no JWT context)
- Service worker forwards multipart data to new `POST /api/v1/spaces/{spaceId}/share` backend endpoint
- Backend validates PIN, creates item, broadcasts via SignalR
- Service worker redirects to space view

**Decision 3: File Types**
Accept all file types (`*/*`) in manifest. Let server quota enforcement and user storage limits handle restrictions. SharedSpaces is a generic hub; any file should be valid.

**Decision 4: MVP Authentication Model**
Use **Option A (Automatic Anonymous Member)** for MVP simplicity:
- Share target POST includes PIN (like token exchange)
- Server validates PIN and auto-creates temporary SpaceMember
- No schema changes; reuses existing quota and file storage
- Backend effort: 2-3 hours

#### Rationale
- Service worker for offline + share target is a natural fit in Phase 5 ("polish")
- PIN-based auth mirrors existing token exchange flow
- Anonymous member simplifies MVP; can refine in future iteration
- Keeping both features together reduces service worker maintenance burden

#### Blockers / Questions
1. **Member attribution:** Should shares create real members or system member?
2. **User auth state:** Pre-joined users only, or support new user onboarding?
3. **Mobile priority:** Android-first or include iOS fallback?
4. **File size limits:** Accept all or add MIME type restrictions?
5. **Service worker scope:** Single unified SW or separate registrations?
6. **Post-share UX:** Auto-navigate, toast notification, or silent?

#### Next Steps
- Marek (product) answers 6 architectural questions
- Reassign to Kaylee (backend) + Wash (frontend) for parallel implementation
- Kaylee: Implement `/api/v1/spaces/{spaceId}/share` endpoint
- Wash: Create manifest.json, service worker, share-accept UI component

---

### Issue #42: Share Target Frontend Requirements & Data Flow
...
#### Context
Frontend must deliver manifest.json, service worker, and share-accept component to enable Web Share Target functionality. Data flow requires careful handling of unauthenticated shares and multi-server state.

#### Decisions

**Decision 1: Method & Multipart**
Use POST with multipart/form-data encoding. Supports both text and files, aligns with backend expectations, better mobile UX than GET query params.

**Decision 2: Service Worker Scope**
Build minimal service worker for share target only (MVP). Defer full offline support + caching strategies to Phase 5 issue #28. Single concern per implementation.

**Decision 3: Unauthenticated Share Handling**
Use **Option C (Hybrid):** Store share data temporarily, redirect to join-view if user not authenticated, re-offer sharing after join completes. Balances UX with implementation complexity.

**Decision 4: Space Selection UI**
Always show space picker (no auto-select). Users maintain mental model of which space shares go to. Can add "Remember my choice" checkbox later for power users.

**Decision 5: Temporary Storage**
Use sessionStorage for MVP. Simple, synchronous, survives navigation but not app close. Sufficient for share-accept ephemeral data. Migrate to IndexedDB in Phase 5 for offline persistence.

**Decision 6: Build Integration**
Separate Vite entry point for service worker (`src/service-worker.ts` → `public/service-worker.js`). Standard practice, easier to maintain than inlining.

**Decision 7: Content Display**
Show full text inline (user can review before sending). Show file previews only (name + size, optional thumbnails). Text is typically small; files might be huge.

#### Rationale
- POST + multipart is Web Share Target standard; no workarounds needed
- Minimal SW keeps MVP focused; defer complexity to Phase 5
- Hybrid auth handles both existing members + new user discovery
- Always-show space picker prevents accidental sharing to wrong space
- sessionStorage sufficient for ephemeral share data; IndexedDB adds complexity

#### Browser Support
- Chrome 76+ (Android) — ✅ Full support
- Chrome 89+ (Desktop) — ✅ Full support
- Edge 79+ — ✅ Full support
- Firefox — ⚠️ No file support
- Safari/iOS — ❌ No Web Share Target API

**Implication:** Chrome/Edge only. iOS users won't see app in share menu. Document as known limitation.

#### Blockers / Questions
1. How to integrate unauthenticated shares with existing join-view UX?
2. Multi-server support: Always show all servers, or filter to default?
3. Should shared items render differently (highlight, show source)?
4. Any file size validation on client before POST?
5. Should app offer reverse share (Web Share API) as complement?

#### Implementation Sequencing
- **Immediate:** manifest.json, service worker, Vite config
- **Near-term:** share-accept component, route integration, sessionStorage handoff
- **Future (Phase 5):** IndexedDB persistence, offline queueing, service worker caching

#### Next Steps
- Confirm unauthenticated flow design
- Implement service worker with fetch handler for `/share-receive`
- Build share-accept-view component with space selector
- Test on Android Chrome with real file shares

---

### Issue #42: Share Target Backend Architecture & Endpoint Design
...
#### Context
Web Share Target invokes manifest's `action` URL via POST without JWT context (fresh window/tab). Cannot reuse existing `PUT /v1/spaces/{spaceId}/items/{itemId}` endpoint which requires authenticated member identity. Need dedicated share endpoint.

#### Decisions

**Decision 1: Dedicated Share Endpoint**
Create `POST /v1/spaces/{spaceId}/share` (not retrofitted to items endpoint).

**Decision 2: Authentication — Option A (Anonymous Member)**
For MVP simplicity:
- Share target POST includes PIN (validates like token exchange)
- Server auto-creates temporary SpaceMember with synthetic name (e.g., "Shared via OS")
- Item created and attributed to this member
- No schema changes; reuses existing quota enforcement
- Simple implementation (2-3 hours backend work)

**Decision 3: Endpoint Response**
- 201 Created with item details + space context
- 404 if space not found
- 401 if PIN invalid
- 413 if space quota exceeded
- Item broadcast via SignalR to connected clients

#### Rationale
- PIN-based validation mirrors existing `POST /v1/spaces/{spaceId}/tokens` flow
- Anonymous member avoids schema migration while preserving audit trail
- Dedicated endpoint keeps concerns separated (auth flow vs. share flow)
- Server generates item GUID (client can't provide it in share context)

#### Architecture Notes
- Service worker cannot directly call `/api/v1/spaces/{spaceId}/items/` (no JWT)
- Multipart form parsing already supported via `IFormFile`
- File storage `IFileStorage.SaveAsync()` handles quota enforcement
- Static manifest serving: Vite bundles `manifest.json` into `dist/`

#### Blockers / Questions
1. **Member attribution:** Temporary members or refactor to nullable MemberId?
2. **Share entry point:** Should manifest point to backend endpoint or SPA route?
3. **PIN reusability:** Can PIN be used multiple times, or one-time only?
4. **Quota validation:** Same per-space quota as regular items, or separate limit?
5. **Static file serving:** Does ASP.NET need `UseStaticFiles()` or sufficient via Vite?

#### Two-Phase Implementation

**Phase 1 (MVP) — Issue #42**
- `POST /v1/spaces/{spaceId}/share` endpoint
- Accept PIN, title, text, url, files
- Option A authentication (automatic anonymous member)
- Reuse quota enforcement + file storage
- Server generates item GUID
- Broadcast via SignalR
- **Effort:** 2-3 hours backend, can run parallel with frontend

**Phase 2 (Polish) — Issue #28**
- Service worker offline queueing
- IndexedDB share data persistence
- Sync logic when online

#### Next Steps
- Marek clarifies member attribution + PIN reusability
- Implement `POST /v1/spaces/{spaceId}/share` endpoint
- Add integration tests: happy path, PIN invalid, quota exceeded
- Coordinate with Wash (frontend) on multipart field names, error codes

---

### PR #45 Review: Consistent 401 Handling & Lit Lifecycle
...
#### Context
PR #45 (squad/25-space-view-items) required standardizing error handling and eliminating duplicate data fetches due to Lit lifecycle race conditions.

#### Decisions

**Decision 1: All API Handlers → 401 Redirect**
Every handler in `space-view.ts` that calls the API checks for `SpaceApiError` with status 401 and redirects to join-view via `redirectToJoin()`. Matches admin panel's 401 → login bounce pattern and prevents users getting stuck with generic error messages.

**Decision 2: Lit `updated()` as Sole Data Fetch Trigger**
Removed `loadData()` calls from `connectedCallback()`. The `updated()` hook is the single entry point for all data loading, guarded by `lastLoadedKey` string (`serverUrl|spaceId`) to prevent duplicate fetches. Eliminates race condition where both hooks fire on first mount.

**Decision 3: Static Keyboard Shortcut Hint**
Text input hint uses `Ctrl/⌘+Enter` as static string rather than runtime platform detection. Simpler, covers both platforms, avoids user-agent sniffing.

#### Rationale
- Consistent 401 handling improves user experience; no stalled views
- Single data-fetch entry point reduces bugs from lifecycle misunderstanding
- Static hints keep code simple; runtime platform detection adds complexity without UX benefit

#### Impact
- 401 errors gracefully handled across all API calls
